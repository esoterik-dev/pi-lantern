...
import { providers, statusEntry } from "../../lantern-state.ts";
...
// Rows that the statusline normally shows but drops while the sidebar is
// visible: cache countdown, warm-cache info, codex quota, conversation cost.
function renderHarnessRows(ctx: SidebarContext, width: number): string[] {
  const lines: string[] = [];
  const valWidth = Math.max(0, width - label("cache").length);

  if (ctx.cacheLastActive !== null) {
    const remainingMs = CACHE_TTL_MS - (Date.now() - ctx.cacheLastActive);
    if (remainingMs > 0) {
      const s = Math.ceil(remainingMs / 1000);
      const text = `⏳ ${Math.floor(s / 60)}:${String(s % 60).padStart(2, "0")}`;
      const tone = remainingMs > 60_000 ? COLORS.success : COLORS.warning;
      lines.push(dim(label("cache")) + fg(tone, text));
    }
  }

  const warm = statusEntry("pi-warm-cache");
  if (warm) {
    const bad = /error|fail/i.test(warm);
    const text = trunc(warm, valWidth);
    lines.push(dim(label("warm")) + fg(bad ? COLORS.error : COLORS.muted, text));
  }

  const quota = providers.getQuotaText?.();
  if (quota) {
    lines.push(dim(label("quota")) + fg(COLORS.muted, trunc(quota, valWidth)));
  }

  const cost = providers.getCostText?.();
  if (cost) {
    lines.push(dim(label("cost")) + fg(COLORS.success, trunc(cost, valWidth)));
  }
...
export function renderSessionPanel(ctx: SidebarContext, width: number): string[] {
  ...
  // Cache / warm / quota / cost (moved from the statusline while the sidebar is shown)
  lines.push(...renderHarnessRows(ctx, width));

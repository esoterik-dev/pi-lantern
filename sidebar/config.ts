...
export interface SidebarSettings {
  ...
  todosMax: number;
  /** Minimum terminal columns the main area must keep while the sidebar is shown. When the
   *  terminal is narrower than width + 1 + minMainWidth the sidebar is hidden so the statusline
   *  keeps quota/cost/cache/warm segments at full width. */
  minMainWidth: number;
}

export const DEFAULT_SIDEBAR_SETTINGS: SidebarSettings = { enabled: true, width: 45, todosMax: 10, minMainWidth: 72 };
export const MIN_SIDEBAR_WIDTH = 10;
export const MAX_SIDEBAR_WIDTH = 120;
export const MIN_MAIN_WIDTH = 40;
export const MAX_MAIN_WIDTH = 200;
...
export function loadSidebarSettings(path: string = sidebarConfigPath()): SidebarSettings {
  ...
  const mmw = obj["minMainWidth"];
  if (typeof mmw === "number" && Number.isInteger(mmw) && mmw >= MIN_MAIN_WIDTH && mmw <= MAX_MAIN_WIDTH) {
    settings.minMainWidth = mmw;
  }
  return settings;
}

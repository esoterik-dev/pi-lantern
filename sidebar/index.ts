...
import { loadSidebarSettings, getAutoCompactEnabled, saveSidebarSettings, MIN_TODOS_MAX, MAX_TODOS_MAX, MIN_MAIN_WIDTH, MAX_MAIN_WIDTH } from "./config.ts";
...
let minMainWidth = initialSettings.minMainWidth;

// Mirror of timestamp-pi's cache countdown so the sidebar can show the same
// "cache ablaufzeit" row the statusline drops while the sidebar is visible.
const CACHE_TTL_MS = 5 * 60 * 1000;
let cacheLastActive: number | null = null;
...
export default function piSidebar(pi: ExtensionAPI) {
  ...
  const startCacheTimer = () => {
    ...
    cacheTimer = setInterval(() => {
      if (cacheLastActive === null || Date.now() - cacheLastActive > CACHE_TTL_MS) {
        if (cacheTimer) { clearInterval(cacheTimer); cacheTimer = null; }
        cacheLastActive = null;
        requestRender?.();
        return;
      }
      requestRender?.();
  ...
  const composeCompositor = (tui: any) => {
  ...
  const setSidebarEnabled = (enabled: boolean, ctx: any) => {
    ...
    saveSidebarSettings({ enabled: sidebarEnabled, width: sidebarWidth, todosMax, minMainWidth });

    if (!sidebarEnabled) {
      compositorRef?.dispose();
      compositorRef = null;
    } else if (tuiRef && !compositorRef) {
      const comp = composeCompositor(tuiRef);
      compositorRef = comp;
      requestRender?.();
    }
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    { let inSum = 0, outSum = 0, cacheSum = 0, costSum = 0, turns = 0;
      ...
      if (cacheSum > 0) { cacheLastActive = Date.now(); startCacheTimer(); }
      turnCount = turns;
    }
    ...
    ui.setWidget("pi-sidebar", (tui: any, _theme: any) => {
      ...
      if (sidebarEnabled) {
        const comp = composeCompositor(tui);
        compositorRef = comp;
      }
  ...
  pi.registerCommand("pi-lantern", {
    description: "Control sidebar: /pi-lantern on | off | width <N> | minmain <N> | todos <N>",
    handler: async (args, ctx) => {
      ...
      if (cmd === "width") {
        ...
        if (isNaN(n) || n < 10 || n > 120) {
          (ctx as any).ui?.notify?.("Usage: /pi-lantern width <10-120>", "warning");
          return;
        }
        sidebarWidth = n;
        saveSidebarSettings({ enabled: sidebarEnabled, width: sidebarWidth, todosMax, minMainWidth });
      ...
      if (cmd === "minmain") {
        const n = parseInt(parts[1] ?? "", 10);
        if (isNaN(n) || n < MIN_MAIN_WIDTH || n > MAX_MAIN_WIDTH) {
          (ctx as any).ui?.notify?.(`Usage: /pi-lantern minmain <${MIN_MAIN_WIDTH}-${MAX_MAIN_WIDTH}>`, "warning");
          return;
        }
        minMainWidth = n;
        saveSidebarSettings({ enabled: sidebarEnabled, width: sidebarWidth, todosMax, minMainWidth });
        requestRender?.();
        (ctx as any).ui?.notify?.(`Min main width set to ${n}`, "info");
        return;
      }
      ...
      if (cmd === "todos") {
        ...
        if (isNaN(n) || n < MIN_TODOS_MAX || n > MAX_TODOS_MAX) {
          (ctx as any).ui?.notify?.(`Usage: /pi-lantern todos <${MIN_TODOS_MAX}-${MAX_TODOS_MAX}>`, "warning");
          return;
        }
        todosMax = n;
        saveSidebarSettings({ enabled: sidebarEnabled, width: sidebarWidth, todosMax, minMainWidth });
      ...
      if (cmd !== "on" && cmd !== "off") {
        (ctx as any).ui?.notify?.("Usage: /pi-lantern on | off | width <N> | minmain <N> | todos <N>", "warning");
        return;
      }

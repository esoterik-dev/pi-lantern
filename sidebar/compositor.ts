...
import { setSidebarActive } from "../lantern-state.ts";
...
export class SidebarCompositor {
  ...
  private readonly minMainWidth: number;
  private readonly onVisibilityChange?: (active: boolean) => void;
  ...
  constructor(
    ...
    this.minMainWidth = minMainWidth;
  ...
  install(): void {
    ...
    Object.defineProperty(terminal, "columns", {
      ...
      get() {
        ...
        return self.isCurrentlyActive() ? Math.max(1, raw - self.sidebarWidth - 1) : Math.max(1, raw);
      },
    });
  ...
  // The sidebar is only shown when the terminal leaves enough room for the
  // main area. Below that the sidebar is hidden so the statusline gets the
  // full width back (quota/cost/cache/warm segments stay visible there).
  isCurrentlyActive(): boolean {
    if (this.disposed) return false;
    return this.rawColumns() >= this.sidebarWidth + 1 + this.minMainWidth;
  }
  ...
  private mainWidth(): number {
    if (!this.isCurrentlyActive()) return this.rawColumns();

...
import { codexQuotaText, renderCodexQuota } from "./codex-quota-display.ts";
import { componentColor } from "./component-colors.ts";
import type { SeparatorStyle, ZentuiConfig } from "./config.ts";
...
import { truncateFooterText } from "./footer-text.ts";
import {
...
import { providers, setSidebarActive, sidebarActive } from "../lantern-state.ts";
...
export function installFooter(
	...
	ctx.ui.setFooter((tui, theme, footerData) => {
		...
		providers.getStatusMap = () => footerData.getExtensionStatuses();
		...
		return {
			dispose: () => {
				...
				providers.getStatusMap = null;
				providers.getQuotaText = null;
				providers.getCostText = null;
			...
			render: function renderFooter(width: number, showQuota = true): string[] {
				...
				// While the sidebar is visible the cache/warm/quota/cost info moves into
				// the sidebar's Session panel, so the statusline drops those segments
				// and hands its full width to the main area.
				const suppress = sidebarActive;
				providers.getStatusMap = () => footerData.getExtensionStatuses();
				const quota =
					ctx.model?.provider === "openai-codex" ? hooks.getCodexQuota?.() : undefined;
				const styledQuota =
					showQuota && !suppress ? renderCodexQuota(quota, theme, config, "footer") : "";
				providers.getQuotaText = () => (quota ? codexQuotaText(quota) : "");
				...
				const builtInCostLabel = [
					...
					.filter(Boolean)
					.join(" ");
				providers.getCostText = () =>
					state.costLabel ? `${state.costLabel}${state.subscription ? " (sub)" : ""}` : "";
				const extensionStatuses = collectExtensionStatusSegments(
					suppress
						? (() => {
								const filtered = new Map<string, string>();
								for (const [key, value] of footerData.getExtensionStatuses()) {
									if (key === "cache-timestamp-pi" || key === "pi-warm-cache") continue;
				...
				const rightParts = [
					...
					config.components.footer.styles.starship.segments.cost && !suppress ? builtInCostLabel : "",
					timeSegment,
				];
				if (styledQuota) {
